...
if upload_file is not None:
    ...
    st.sidebar.write("weapons counts",weapons_counts)
    ...
    with col2:
        injury_type=df['type_of_injury'].value_counts()
        st.subheader('Type of Injury')
        fig,ax=plt.subplots()
        ax.pie(injury_type,labels=injury_type.index,autopct='%1.1f%%')
        st.pyplot(fig)
    ...
    col1,col2=st.columns(2)
    with col1:
        incidentcntbyreg=df.groupby('citizenship').size().reset_index(name='incident_count')
        st.subheader('Incident count by nationality')
        st.write(incidentcntbyreg)
    with col2:
